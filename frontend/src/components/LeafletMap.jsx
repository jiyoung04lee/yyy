...
const LeafletMap = ({ minSheetHeight, headerHeight, parties }) => { // parties prop 추가
  ...
  useEffect(() => {
    ...
    if (mapRef.current) {
      ...
      const zoomOffset = 0; // 초기 줌 오프셋 (0: 높이 맞춤, 양수: 확대, 음수: 축소)
      ...
      mapInstance.current = L.map(mapRef.current, {
        ...
        maxBounds: bounds, // 맵 이동을 이미지 경계로 제한 (메인 이미지 기준)
  ...
  useEffect(() => {
    ...
    parties.forEach(party => {
      ...
      const pixelY = party.place_y_norm * imageHeight;
      const pixelX = party.place_x_norm * imageWidth;
